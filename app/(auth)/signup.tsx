...
export default function SignupScreen() {
  ...
  function withReferral<T extends object>(submit: (data: T & { referralCode?: string }) => void) {
    return (data: T) => {
      const referral = signupReferralCode(referralCode);
      if (!referral.ok) {
    ...
    return (
      <AuthShell>
        ...
        <Text className="text-center text-sm text-gray-500">
          ...
          <Text className="font-semibold text-gray-900">{signedUpEmail}</Text>. Please check your
          inbox and tap the link to activate your account.
        </Text>
  ...
  return (
    <AuthShell>
      <View className="space-y-1">
        <Text className="text-center text-3xl font-extrabold text-gray-900">Create Account</Text>
        <Text className="text-center text-sm text-gray-500">
